...
int Nk=4;
unsigned char in_aes[16], out_aes[16], state[4][4];
...
{
	int i,j;
	for(i=0;i<4;i++) for(j=0;j<4;j++) state[i][j] = getSBoxInvert(state[i][j]);
}
...
{
	int i,j;
	for(i=0;i<4;i++) for(j=0;j<4;j++) state[i][j] = getSBoxValue(state[i][j]);
}
...
//	for(i=0;i<16;i++) Key[i]=temp[i];
	...
	for(i=0;i<16;i++) in_aes[i]=temp2[i+j*16];
	...
	for(i=0;i<16;i++)
	{
		printf("%02x ",out_aes[i]);
	}
	printf("\n\n");
	
	//------------------decrypt-------------------
	
	for(i=0;i<16;i++) in_aes[i]=out_aes[i];
